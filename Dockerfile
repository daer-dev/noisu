...
# The name of the application is noisu and while there
...
# having to CD into the /noisu folder every time would be
# lame, so instead we can set the WORKDIR to be /noisu.
#
...
# noisu/ path inside of the Docker image.
#
# It copies it to /noisu because of the WORKDIR being set.
#
...
# /noisu folder inside of the Docker image.
#
